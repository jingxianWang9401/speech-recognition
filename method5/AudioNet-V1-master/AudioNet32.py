...
def read_aud(path):
    ...
    pad_array=np.zeros(35200-(audio.shape[0]))
    pad_aud=np.hstack((audio,pad_array))
    
    #noise= np.random.randint(low=-200,high=200, size=16000)
    noise= np.random.randint(low=-200,high=200, size=35200)
...
def model_init():
    ...
    model.add(Conv1D(filters=32, kernel_size=32, strides=4, padding='valid', activation='relu',input_shape=(35200,1)))
    model.add(Conv1D(filters=64, kernel_size=32, strides=4, padding='valid', activation='relu', kernel_initializer='glorot_normal'))
    ...
    model.add(Conv1D(filters=512, kernel_size=32, strides=4, padding='valid', activation='relu', kernel_initializer='glorot_normal'))
#    model.add(Conv1D(filters=128, kernel_size=32, strides=4, padding='valid', activation='relu', kernel_initializer='glorot_normal'))
    ...
    with open("E:/speech_rocognition_demo/AudioNet-V1-master/trained_models/model_32.yaml", "w") as json_file:
...
def validate_model(val,val_gt,model,val_samples):
    ...
    validation_accuracy=accuracy_score(np.argmax(predictions, axis=1), mt_gt)
    
    validationtxt=str(validation_accuracy)          # data是前面运行出的数据，先将其转为字符串才能写入
    with open('E:\\speech_rocognition_demo\\AudioNet-V1-master\\result\\validation_accuracy.txt','a') as file_handle:
...
def resume_training(model,weights):
    ...
    with open('E:/speech_rocognition_demo/AudioNet-V1-master/trained_models/train_data_dic.pkl', 'rb') as handle:
...
def main():
    ...
    data_path='E:/speech_rocognition_demo/AudioNet-V1-master/dataset/original/'
    train_ratio=0.8
    ...
    model=model_init()
    csv_logger = keras.callbacks.CSVLogger('E:/speech_rocognition_demo/AudioNet-V1-master/result/training.log',append=True)
...
#num_epochs    
    ...
    for e in range(50):
        ...
        validate_model(val,val_gt,model,512)
        
            
        if e%2==0:
            
            model.save('E:/speech_rocognition_demo/AudioNet-V1-master/trained_models/epoch_'+str(e)+'.h5')
 
    model.save('E:/speech_rocognition_demo/AudioNet-V1-master/trained_models/epoch_'+str(e)+'_final.h5')
